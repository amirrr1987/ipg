<template>
  <Form layout="vertical">
    <FormItem label="شماره کارت">
      <!-- {{ validatePanCard(card.panNumber) }} -->
      <Input v-model:value="panNumber" size="large" class="ltr h-14" />
    </FormItem>
    <FormItem label="شماره شناسایی دوم (CVV2)">
      <Input
        type="password"
        v-model:value="cvv2"
        size="large"
        class="ltr text-center h-14 text-xl text-gray-500"
      />
    </FormItem>
    <FormItem label="تاریخ انقضا">
      <div class="grid grid-cols-2 gap-8">
        <Input v-model:value="month" class="w-full ltr text-center h-14" size="large" />
        <Input v-model:value="year" class="w-full ltr" size="large" />
      </div>
    </FormItem>

    <div class="grid grid-cols-2 gap-8"></div>
    <FormItem label="کد امنیتی">
      <div class="grid grid-cols-2 gap-8">
        <Input v-model:value="card.captcha" size="large" class="ltr text-center h-14" />
        <img
          src="data:image/JPEG;base64,"
          alt=""
          srcset=""
        />
      </div>
    </FormItem>
    <FormItem label="رمز دوم">
      <div class="grid grid-cols-2 gap-8">
        <Input type="password" size="large" class="ltr h-14" />
        <Button type="primary" size="large" class="!h-14">درخواست رمز دوم</Button>
      </div>
    </FormItem>
    <FormItem label="ایمیل یا شماره تلفن همراه">
      <Input size="large" class="ltr h-14" />
    </FormItem>
    <FormItem>
      <div class="grid grid-cols-2 gap-8">
        <Button type="primary" block size="large" class="!h-14">پرداخت</Button>
        <Button size="large" block class="!h-14">انصراف</Button>
      </div>
    </FormItem>
  </Form>
</template>
<script setup lang="ts">
import { Form, FormItem, Input, InputNumber, Button } from 'ant-design-vue/es'
import { computed, ref } from 'vue'
import { validatePanCard } from '@/utils'
const card = ref({
  panNumber: '',
  cvv2: '',
  month: '',
  year: '',
  captcha: ''
})

const panNumber = computed({
  set: (value) => {
    card.value.panNumber = value.replace(/[^\d]/g, '').slice(0, 16)
  },
  get: () => {
    let x = card.value.panNumber.replace(/[^\d]/g, '')
    return x.replace(/(\d{4}(?=\d))/g, '$1 ')
  }
})

const cvv2 = computed({
  set: (value) => {
    card.value.cvv2 = value.replace(/[^\d]/g, '').slice(0, 4)
  },
  get: () => {
    return card.value.cvv2
  }
})

const month = computed({
  set: (value) => {
    card.value.month = value.replace(/[^\d]/g, '').slice(0, 2)
  },
  get: () => {
    return card.value.month
  }
})

const year = computed({
  set: (value) => {
    card.value.year = value.replace(/[^\d]/g, '').slice(0, 2)
  },
  get: () => {
    return card.value.year
  }
})

const enforcePANFormat = () => {}
</script>
<style lang="less">
:where(.ant-form-row) {
  // @apply !grid gap-4;
  // grid-template-columns: 100px 1fr;
  // :where(.ant-form-item-label) {
  //   @apply !text-right;
  // }
}
</style>
